...
<head>
    ...
    <title>Recuperar Senha — Cursos GT</title>
...
<body class="antialiased min-h-screen flex items-center justify-center bg-radial-glow h-screen select-none">
    ...
    <main class="w-full max-w-[440px] p-6 z-10 m-auto">
        ...
        <div class="glass-card rounded-xl p-10">
            <!-- Logo / Header -->
            <div class="text-center mb-6" id="recoverHeader">
                ...
                <p class="text-text-muted mt-2 text-xs font-semibold uppercase tracking-wider">Insira seu e-mail para receber as instruções</p>
            </div>
            ...
            <form id="recoverForm" class="space-y-6">
                ...
                <div class="input-group">
                    <label class="input-label" for="email">E-mail Cadastrado</label>
                ...
                <div class="hidden text-error text-xs font-semibold mt-2" id="error-message">
                    <span class="flex items-center gap-2">
                        ...
                        <span id="error-text">Ocorreu um erro ao processar.</span>
                    </span>
                </div>
                ...
                <button class="w-full btn-primary font-bold py-4 rounded-lg uppercase tracking-[0.15em] text-[12px] mt-2 flex items-center justify-center gap-2" type="submit" id="submitBtn">
                    <span id="btnText">Solicitar Recuperação</span>
                    <!-- Loading Spinner -->
            ...
            <div class="hidden text-center py-6 space-y-4" id="successState">
                <span class="material-symbols-outlined text-6xl text-primary drop-shadow-[0_0_15px_rgba(241,200,75,0.4)]">mark_email_read</span>
                <h2 class="text-lg font-bold text-text-main uppercase">E-mail Enviado!</h2>
                <p class="text-xs text-text-muted font-medium leading-relaxed">
                    Se este endereço estiver associado a uma conta ativa, enviamos as instruções de recuperação. Por favor, verifique sua caixa de entrada e spam.
                </p>
                <div class="pt-4 debug-container hidden" id="devDebugContainer">
                    <p class="text-[10px] text-primary uppercase font-bold tracking-widest">[DEBUG LINK - DEV MODE]</p>
                    <a class="text-xs text-white underline break-all font-mono" id="devDebugLink" href="#" target="_blank"></a>
                </div>
            </div>
            ...
            <div class="text-center mt-8">
                <p class="text-xs text-text-muted font-semibold uppercase tracking-wider">
                    Lembrou da senha? 
                    <a class="text-primary hover:text-[#FFD700] font-bold transition-colors ml-1 uppercase tracking-widest" href="login.php">
                        Voltar ao Login
                    </a>
                </p>
            </div>
    ...
    <script>
        document.getElementById('recoverForm').addEventListener('submit', function(e) {
            ...
            const recoverHeader = document.getElementById('recoverHeader');
            ...
            .then(data => {
                if (data.success) {
                    // Oculta cabeçalho e formulário original
                    recoverHeader.classList.add('hidden');
                    recoverForm.classList.add('hidden');
                    
                    // Exibe a tela de sucesso
                    successState.classList.remove('hidden');

                    // Se estiver em modo DEV e houver um link de depuração retornado
                    if (data.debug_link) {
                        document.getElementById('devDebugContainer').classList.remove('hidden');
                        const dbg = document.getElementById('devDebugLink');
                        dbg.href = data.debug_link;
                        dbg.textContent = data.debug_link;
                    }
                }
            })
            .catch(error => {
                ...
                btnText.textContent = 'Solicitar Recuperação';
